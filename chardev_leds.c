...
device_write(struct file *filp, const char *buff, size_t len, loff_t * off)
{
	...
	//Copy_from_user devuelve el numero de bytes que NO pudieron copiarse
	if(copy_from_user(buffAux, buff, len) == 0){
		int i;
		for(i = 0; i < len; i++){
			if(buffAux[i] == '1'){
				leds = leds | 0x2;  //Se hace una OR con 010, el bit del medio coincide con el led num_lock
			}
			else if(buffAux[i] == '2'){
				leds = leds | 0x4; //Se hace una OR con 100 el bit de la izquierda coincide con el led caps_lock
			}
			else if(buffAux[i] == '3'){
				leds = leds | 0x1; //Se hace una OR con 001, el bit de la derecha coincide con el led scroll_lock
			}
		}
